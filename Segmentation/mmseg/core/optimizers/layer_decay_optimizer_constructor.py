...
def get_num_layer_stage_wise(n, num_max_stage=12):
    # def get_layer_id(n, dp, layers):
    if n.startswith("backbone.subnet"):
        n=n[9:]
        name_part = n.split('.')
        subnet = int(name_part[0][-1])
        if name_part[1].startswith("alpha") or name_part[1].startswith("level"):
            id = int(name_part[1][-1])+1
            # id = dp[block][subnet]
    elif n.startswith("backbone.stem"):
        id = 0
    else:
...
@OPTIMIZER_BUILDERS.register_module()
class LearningRateDecayOptimizerConstructorRevCol(DefaultOptimizerConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        decay_rate = self.paramwise_cfg.get('decay_rate')
        decay_type = self.paramwise_cfg.get('decay_type', "layer_wise")
        print("Build LearningRateDecayOptimizerConstructor %s %f" % (decay_type, decay_rate))
        weight_decay = self.base_wd
        ...
        for name, param in module.named_parameters():
            ...
            if decay_type == "stage_wise":
                layer_id = get_num_layer_stage_wise(name, num_max_stage=12)
                num_layers = 12
